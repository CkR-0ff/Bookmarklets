var ElementRemover = function(){/*javascript: if (!dieFunc) {
...
var YTAutoReplay = function(){/*javascript: console.log(myIntrID);
var myIntrID = myIntrID;
var repRun = function() {
    if (!myIntrID) {
        myIntrID = setInterval(function() {
            var btns = document.getElementsByClassName('ytp-play-button ytp-button');
            console.log(btns.length);
            for (var i = 0; i < btns.length; i++) {
                if (btns[i].title == "Replay" || btns[i].title == "Пускане пак") {
                    btns[i].click();
                }
            }
        }, 2000);
    } else {
        clearInterval(myIntrID);
        myIntrID = undefined;
    }
};
repRun();*/}.toString().slice(14, -4);
...
function RetJson(){
  var obj = {
	...
	"Code": ElementRemover, 
	"Description": "When activated the hovered element is outlined in red and his name is shown. On Delete button press the element is removed. The functionality is removed on second click of the bookmarklet."
      },
      {
	"Title": "CSS Hider", 
	"Code": CSSHider, 
	...
	"Description": "Automaticaly downloads all post pictures(images, WEBMs, GIFs, etc.) in a 4chan thread. Creates and downloads 'NamesLinks.txt containig the coresponding String names to every code name of every picture.'"
      ...
      {
	"Title": "YouTube Auto Repaly", 
	"Code": YTAutoReplay, 
	"Description": "When activated on YouTube video page the video is automatically replayed when it finishes(works only on english and bulgarian languages)."
      }
